...
namespace AB
{
	RenderGroup* RenderGroupAllocate(Memory* mem,
									 u32 rbSize,
									 u32 queueCapacity,
									 u32 lightBufCapacity)
	{
		...
		group->commandQueue = (CommandQueueEntry*)SysAllocAligned(commandQueueSize,
																  alignof(CommandQueueEntry));
		AB_CORE_ASSERT(group->commandQueue, "Failed to allocate render queue");
		group->tmpCommandQueue = (CommandQueueEntry*)SysAllocAligned(commandQueueSize,
																	 alignof(CommandQueueEntry));
		...
		group->pointLights = (PointLight*)SysAllocAligned(lightBufSize,
														  alignof(PointLight));
...
#define RENDER_KEY_INSERT_DEPTH(key, depth)		\
	...
	static inline u64 _MakeKeyMesh(BlendMode blendMode,
								   ...
								   v3 cameraPosition)
	{
		...
		{
			Mesh* mesh = AssetGetMeshData(PermStorage()->asset_manager, meshHandle);
								...
								void* data)
	{
		...
		switch (type)
		{
			case RENDER_COMMAND_DRAW_MESH:
			{
				...
				u64 sortKey =  _MakeKeyMesh(renderData->blendMode,
