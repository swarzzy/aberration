...
namespace AB
{
	...
	{
		return CreateTexture(p, img->format, img->width,
							 img->height, img->bitmap);	
	}
	...
	void BindFramebuffer(u32 handle, FramebufferTarget type)
	{
								  ...
								  u32 textureHandle)
	{
		...
		GLCall(glFramebufferTexture2D(glTarget, glAtt, GL_TEXTURE_2D,
									  textureHandle, 0));
	}
													...
													u32 textureHandle)
	{
		auto glAtt = _OpenglResolveFramebufferAttachment(att);
		auto glTarget = _OpenglResolveFramebufferTarget(target);
		GLCall(glFramebufferTexture2D(glTarget, glAtt, GL_TEXTURE_2D_MULTISAMPLE,
									  textureHandle, 0));	
	}
