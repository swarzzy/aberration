...
namespace AB {
	...
	struct InputMgr {
		...
		i32 mouseScrollOffset;
		i32 mouseFrameScrollOffset;
	...
	AB_API InputMgr* InputInitialize(MemoryArena* memory, PlatformState* platform);
	...
	inline i32 InputGetScrollOffset(InputManager* mgr)
	{
		return mgr->mouseScrollOffset;	
	}
	
	inline i32 InputGetFrameScrollOffset(InputManager* mgr)
	{
		return mgr->mouseFrameScrollOffset;
	}
}
