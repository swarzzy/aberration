...
namespace AB {
	...
	AssetManager* AssetManagerInitialize(MemoryArena* memory,
						  ...
						  uint32* indices, Material* material)
	{
		...
		if (found) {
			...
			mgr->meshes[free_index].mem_begin = (byte*)PushSize(memory, mem_size, 0);
	...
	int32 AssetCreateMeshAAB(AssetManager* mgr, MemoryArena* arena,
							 MemoryArena* temporaryArena, const char* aab_path)
	{
		int32 result_handle = ASSET_INVALID_HANDLE;

		u32 fileSize = DebugGetFileSize(aab_path);
		if (fileSize)
		{
			void* memory = PushSize(temporaryArena, fileSize, 0);
			u32 bytesRead = DebugReadFile(memory, fileSize, aab_path);
			if (bytesRead == fileSize) {
				AABMeshHeader* header = (AABMeshHeader*)memory;
				if (header->magic_value == AAB_FILE_MAGIC_VALUE && header->version == AAB_FILE_VERSION) {
					...
					if (asset_type == AAB_FILE_TYPE_MESH) {
						...
						void* asset_data = (void*)((byte*)memory + vertices_offset);
						...
						if (asset_data) {
							...
							AABMeshMaterialProperties* material_props = has_material ?
								(AABMeshMaterialProperties*)((byte*)asset_data + (material_properties_offset - vertices_offset)) : nullptr;
//
							if (has_material) {
								Material material = {};
								if (material_diff_map_name_offset) {
									...
									material.diff_map_handle = AssetCreateTextureBMP(mgr, arena, temporaryArena,
																					 path_buff, MapType::Diffuse);
								} else {
								...
								if (material_spec_map_name_offset) {
									...
									material.spec_map_handle = AssetCreateTextureBMP(mgr, arena, temporaryArena,
																					 path_buff, MapType::Specular);
								}
								else {
								...
								result_handle = AssetCreateMesh(mgr, arena,  vertices_count, vertices, uvs, normals, indices_count, indices, &material);
							} else {
								result_handle = AssetCreateMesh(mgr, arena, vertices_count, vertices, uvs, normals, indices_count, indices, nullptr);
							}
	...
	int32 AssetCreateTexture(AssetManager* mgr, MemoryArena* memory, byte* bitmap, uint16 w, uint16 h,
		...
		if (found) {
			...
			void* mem_ptr = PushSize(memory, mem_size, 0);
			if (mem_ptr) {
				...
				if (mapType == MapType::Diffuse) {
					cs = TEX_COLOR_SPACE_SRGB;
				} else {
					cs = TEX_COLOR_SPACE_LINEAR;
				}
				
				TextureFormat format = GetTextureFormat(bits_per_pixel, cs);
				
				TextureParameters p = {
					TEX_FILTER_LINEAR,
					TEX_WRAP_CLAMP_TO_EDGE,
				};
				tx->api_handle = CreateTexture(p, format, w, h, bitmap);
				if (!tx->api_handle) {
	...
	int32 AssetCreateTextureBMP(AssetManager* mgr, MemoryArena* memory,
								MemoryArena* tempArena, const char* bmp_path,
								MapType mapType) {
		ColorSpace cs;
		if (mapType == MapType::Diffuse) {
			cs = TEX_COLOR_SPACE_SRGB;
		...
		Image bmp = LoadBMP(tempArena, bmp_path, cs);
		...
		if (bmp.bitmap) {
			...
			return AssetCreateTexture(mgr,memory,  bmp.bitmap, bmp.width, bmp.height,
